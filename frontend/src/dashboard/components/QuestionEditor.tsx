...
import { QuestionType, QuestionTypeLabels } from "../../feedback/types/feedback";
import OptionEditor from "./OptionEditor";
import { requiresOptions, supportsRequired } from "./questionUtils";
...
function QuestionEditor({ question, setForm, form }: Props) {

    const updateQuestion = async (field: keyof Question, value: any) => {
        setForm((prev) => ({
            ...prev,
    ...
    const moveQuestionUp = async () => {
        setForm((prev) => {
            const currentIndex = prev.questions.findIndex((q) => q.id === question.id);
            ...
            return {
                ...prev,
                questions: questions.map((q, index) => ({ ...q, display_order: index + 1 })),
            };
        });
        await fetch(`/api/form/${form.id}/${question.id}/order`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ newOrder: question.display_order - 1 }),
        });
    };
    ...
    const moveQuestionDown = async () => {
        setForm((prev) => {
            const currentIndex = prev.questions.findIndex((q) => q.id === question.id);
            if (currentIndex === prev.questions.length - 1) return prev;
    ...
    const deleteQuestion = async () => {
        ...
        await fetch(`/api/form/${form.id}/${question.id}`, { method: "DELETE" });
    };
    ...
    return (
        <div style={styles.container}>
            ...
            <Select
                ...
                onChange={(value) => updateQuestion("questionType", value as QuestionType)}
            />

            {requiresOptions(question.questionType) && (
                <OptionEditor question={question} setForm={setForm} />
            )}

            {supportsRequired(question.questionType) && (
                <Checkbox
                    isSelected={question.is_required}
                    onChange={(value) => updateQuestion("is_required", value)}
                >
                    Required
                </Checkbox>
            )}
            ...
            <div style={styles.actions}>
                <Button onPress={moveQuestionUp} isDisabled={question.display_order === 1}>
                    ↑ Move Up
                </Button>
                <Button
